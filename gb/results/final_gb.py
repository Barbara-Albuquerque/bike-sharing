import os
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

from scipy.stats import pearsonr

from sklearn.ensemble import HistGradientBoostingRegressor
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import MinMaxScaler
from sklearn.metrics import mean_absolute_error, mean_squared_error
from sklearn.utils import shuffle
from sklearn.linear_model import LinearRegression
from sklearn.inspection import permutation_importance


# ======================================================
# Configuração de saída
# ======================================================
output_dir = "gb/results/hgbr_results"
os.makedirs(output_dir, exist_ok=True)


# ======================================================
# Função auxiliar: RMSE
# ======================================================
def root_mean_squared_error(y_true, y_pred):
    return np.sqrt(mean_squared_error(y_true, y_pred))


# ======================================================
# 1. Leitura dos dados e seleção das variáveis
# ======================================================
df = pd.read_csv("data-processing\imputed-values\SeoulBikeData_clean_imputed.csv", encoding="utf-8")


features = [
    "Hour",
    "Temperature(°C)",
    "Humidity(%)",
    "Wind speed (m/s)",
    "Visibility (10m)",
    "Dew point temperature(°C)",
    "Solar Radiation (MJ/m2)",
    "Snowfall (cm)",
    "Rainfall(mm)",
    "Seasons",
    "Weekday",
    "Day",
    "Month"
]

target = "Rented Bike Count"

X = df[features]
y = df[target]


# ======================================================
# 2. Inicialização das estruturas de métricas
# ======================================================
r_list, rmse_list, mae_list = [], [], []
r_train_list, rmse_train_list, mae_train_list = [], [], []

preds_rows = []
perm_importances = []


# ======================================================
# 3. Step 2 – 10 execuções com splits aleatórios (70/30)
# ======================================================
print("\nSTEP 2 – 10 execuções com divisão 70/30\n")

for i in range(10):

    print(f"Execução {i + 1}/10")

    # Embaralhar dataset completo
    X_shuffled, y_shuffled = shuffle(X, y)

    X_train_raw, X_test_raw, y_train, y_test = train_test_split(
        X_shuffled,
        y_shuffled,
        test_size=0.3
    )

    # --------------------------------------------------
    # Normalização 
    # --------------------------------------------------
    scaler = MinMaxScaler()
    X_train = scaler.fit_transform(X_train_raw)
    X_test = scaler.transform(X_test_raw)

    # --------------------------------------------------
    # Modelo HGBR
    # --------------------------------------------------
    model = HistGradientBoostingRegressor(
        loss="squared_error",
        learning_rate=0.03,
        max_leaf_nodes=63,
        min_samples_leaf=10,
        max_features=0.6,
        max_iter=1000,
        early_stopping=True,
        validation_fraction=0.1,
        n_iter_no_change=30
    )

    # Treinamento
    model.fit(X_train, y_train)

    # --------------------------------------------------
    # Avaliação no treino
    # --------------------------------------------------
    y_pred_train = model.predict(X_train)

    r_train, _ = pearsonr(y_train, y_pred_train)
    rmse_train = root_mean_squared_error(y_train, y_pred_train)
    mae_train = mean_absolute_error(y_train, y_pred_train)

    print(
        f"[TREINO] R={r_train:.4f}, "
        f"RMSE={rmse_train:.2f} cnt/h, "
        f"MAE={mae_train:.2f} cnt/h"
    )

    # --------------------------------------------------
    # Avaliação no teste
    # --------------------------------------------------
    y_pred = model.predict(X_test)

    r, _ = pearsonr(y_test, y_pred)
    rmse = root_mean_squared_error(y_test, y_pred)
    mae = mean_absolute_error(y_test, y_pred)

    print(
        f"[TESTE ] R={r:.4f}, "
        f"RMSE={rmse:.2f} cnt/h, "
        f"MAE={mae:.2f} cnt/h"
    )

    # --------------------------------------------------
    # Permutation Importance (teste)
    # --------------------------------------------------
    result = permutation_importance(
        model,
        X_test,
        y_test,
        n_repeats=10,
        scoring="neg_mean_squared_error",
        random_state=42
    )

    perm_importances.append(result.importances_mean)

    # --------------------------------------------------
    # Armazenar previsões
    # --------------------------------------------------
    preds_rows.append(pd.DataFrame({
        "Execucao": i + 1,
        "Conjunto": "Treino",
        "y_true": y_train.values,
        "y_pred": y_pred_train
    }))

    preds_rows.append(pd.DataFrame({
        "Execucao": i + 1,
        "Conjunto": "Teste",
        "y_true": y_test.values,
        "y_pred": y_pred
    }))

    # --------------------------------------------------
    # Armazenar métricas
    # --------------------------------------------------
    r_train_list.append(r_train)
    rmse_train_list.append(rmse_train)
    mae_train_list.append(mae_train)

    r_list.append(r)
    rmse_list.append(rmse)
    mae_list.append(mae)


# ======================================================
# 4. Resultados médios após 10 execuções
# ======================================================
print("\nMÉDIAS APÓS 10 EXECUÇÕES (TREINO):")
print(f"R médio   : {np.mean(r_train_list):.4f}")
print(f"RMSE médio: {np.mean(rmse_train_list):.2f}")
print(f"MAE médio : {np.mean(mae_train_list):.2f}")

print("\nMÉDIAS APÓS 10 EXECUÇÕES (TESTE):")
print(f"R médio   : {np.mean(r_list):.4f}")
print(f"RMSE médio: {np.mean(rmse_list):.2f}")
print(f"MAE médio : {np.mean(mae_list):.2f}")


# ======================================================
# 5. Função para gráfico predito vs real
# ======================================================
def plot_pred_vs_true(y_true, y_pred, title, ax):

    ax.scatter(y_true, y_pred, color="green", s=5, label="Dados")
    ax.plot([0, max(y_true)], [0, max(y_true)], "k--", label="Ideal")

    reg = LinearRegression().fit(
        y_true.values.reshape(-1, 1),
        y_pred
    )

    fit_line = reg.predict(
        y_true.values.reshape(-1, 1)
    )

    ax.plot(y_true, fit_line, "g-", label="Regressão")

    r, _ = pearsonr(y_true, y_pred)
    rmse = root_mean_squared_error(y_true, y_pred)
    mae = mean_absolute_error(y_true, y_pred)

    ax.text(
        0.05,
        0.9,
        f"R={r:.4f}\nRMSE={rmse:.2f}\nMAE={mae:.2f}",
        transform=ax.transAxes,
        fontsize=9,
        verticalalignment="top"
    )

    ax.set_xlabel("Target values")
    ax.set_ylabel("Output values")
    ax.set_title(title)
    ax.legend()


# ======================================================
# 6. Gráficos treino e teste (última execução)
# ======================================================
fig, axs = plt.subplots(1, 2, figsize=(12, 5))

plot_pred_vs_true(y_train, y_pred_train, "(a) Train data", axs[0])
plot_pred_vs_true(y_test, y_pred, "(b) Test data", axs[1])

plt.tight_layout()
plt.savefig(
    os.path.join(output_dir, "hgbr_step2_pred_vs_true_train_test.png"),
    dpi=300,
    bbox_inches="tight"
)
plt.close()


# ======================================================
# 7. CSVs de métricas por execução
# ======================================================
pd.DataFrame({
    "Execucao": range(1, 11),
    "R": r_train_list,
    "RMSE": rmse_train_list,
    "MAE": mae_train_list
}).to_csv(
    os.path.join(output_dir, "hgbr_step2_treino.csv"),
    index=False
)

pd.DataFrame({
    "Execucao": range(1, 11),
    "R": r_list,
    "RMSE": rmse_list,
    "MAE": mae_list
}).to_csv(
    os.path.join(output_dir, "hgbr_step2_teste.csv"),
    index=False
)


# ======================================================
# 8. Médias em CSV
# ======================================================
pd.DataFrame({
    "Conjunto": ["Treino", "Teste"],
    "R_medio": [np.mean(r_train_list), np.mean(r_list)],
    "RMSE_medio": [np.mean(rmse_train_list), np.mean(rmse_list)],
    "MAE_medio": [np.mean(mae_train_list), np.mean(mae_list)]
}).to_csv(
    os.path.join(output_dir, "hgbr_step2_medias.csv"),
    index=False
)


# ======================================================
# 9. Todas as previsões
# ======================================================
pd.concat(preds_rows, ignore_index=True).to_csv(
    os.path.join(output_dir, "hgbr_step2_preds.csv"),
    index=False
)


# ======================================================
# 10. Feature importance (Permutation – média)
# ======================================================
df_perm = pd.DataFrame(
    perm_importances,
    columns=features
)

df_perm.loc["Média"] = df_perm.mean()

df_perm.to_csv(
    os.path.join(output_dir, "hgbr_feature_importance_permutation.csv")
)


# ======================================================
# 11. Gráfico de Feature Importance
# ======================================================
df_mean_imp = df_perm.loc["Média"].sort_values()

plt.figure(figsize=(8, 5))
plt.barh(df_mean_imp.index, df_mean_imp.values, color="green")
plt.xlabel("Permutation Importance (RMSE)")
plt.title("HGBR Feature Importance (Permutation)")
plt.tight_layout()

plt.savefig(
    os.path.join(output_dir, "hgbr_feature_importance.png"),
    dpi=300,
    bbox_inches="tight"
)

plt.close()
